            ...
            <div class="col-md-5 col-sm-12">
                <div class="form-group text-left">
                    ...
                    <div class="input-group mt-2">
                        ...
                        <input type="text" class="form-control input" value="1000" name="brut" id="brut" onkeypress="return isNumberKey(event)" onchange="stopajHesapla();">
...
@section('scripts')
    ...
    <script>
        ...
        function stopajHesapla(){
            ...
            brut = brut *1 ;
            $('#brut_ucret').html(formatMoney(brut.toFixed(2))+' TL');
            $('#stopaj_orani').html('%'+s_orani);
            $('#gv_stopaji').html(formatMoney((s_orani*brut/100).toFixed(2)) + ' TL');
            $('#net_ucret').html(formatMoney((brut -  (s_orani*brut/100)).toFixed(2)) + ' TL');
            $('#kdv_orani_1').html('%'+kdv);
            $('#kdv_tutari').html(formatMoney((brut*kdv/100).toFixed(2)) + ' TL');
            $('#tahsil_edilen').html(formatMoney(((brut*kdv/100)+ (brut -  (s_orani*brut/100))).toFixed(2)) + ' TL');

            var brut_1 = brut/ (1-(s_orani/100));
            brut_1= brut_1.toFixed(2);

            $('#brut_ucret_1').html( formatMoney(brut_1) + ' TL');
            $('#stopaj_orani_1').html('%'+s_orani);
            $('#gv_stopaji_1').html(formatMoney((brut_1 * s_orani /100).toFixed(2)) + ' TL');
            $('#net_ucret_1').html(formatMoney( brut.toFixed(2))  + ' TL');
            $('#kdv_orani_2').html('%'+kdv);
            var kdv_tutari_1 = (brut_1*kdv/100)*1;
            $('#kdv_tutari_1').html(formatMoney(kdv_tutari_1.toFixed(2))+ ' TL');
            $('#tahsil_edilen_1').html(formatMoney((kdv_tutari_1+brut).toFixed(2)) + ' TL');
...
//B33/(-(B28-B31-1))
            ...
            $('#brut_ucret_2').html( formatMoney(brut_3) +' TL');
            $('#stopaj_orani_2').html('%'+s_orani);
            $('#gv_stopaji_2').html( formatMoney(gv) + ' TL');
            $('#net_ucret_2').html(formatMoney((brut_3-gv).toFixed(2)) + ' TL');
            $('#kdv_orani_3').html('%'+kdv);
            $('#kdv_tutari_tahsil').html(formatMoney((brut_3*kdv/100).toFixed(2))+' TL');
            $('#tahsil_edilen_2').html(formatMoney(tahsil_edilen.toFixed(2)) + ' TL');
