...
class HomeController extends Controller
{
            ...
            $resultArray = DB::transaction(function () use ($request) {
                ...
                if(!empty($checkUser['id'])){
                    ...
                    if($checkUser['active']==1){
                        ...
                        Session::put('user_id',$checkUser['id']);
    ...
    public function  updatePassword(Request $request){
        ...
        $data['who'] = $this->whois(Session::get('user_id'));
        $admin = Admin::find($request->session()->get('user_id'));
    ...
    public function  profile(Request $request){
        ...
        //  $admin = Admin::where('id','=',$request->session()->get('user_id'))->withPivot('clients')->first();
        $admin = Admin::find($request->session()->get('user_id'));
        $data ['admin'] = $admin;
        if ($request->isMethod('post')) {
            $messages = [];
    ...
    public function getPic(){
        $admin = $this->whois(Session::get('user_id'));
